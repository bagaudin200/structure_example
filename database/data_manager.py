...
from handlers import main_request, lowprice, highprice, bestdeal, history
...
def write_data(user_id: int, value: Union[int, str, List[Union[int, float]], Dict[str, Union[str, List[str]]], bool,
    ...
    with open(os.path.join('', str(user_id) + '.json'), 'w') as file:
        i_data[key] = value
...
def read_data(user_id: int) -> Dict[str, Union[int, str, None, List[Union[int, float]], Dict[str,
    ...
    """
    try:
        with open(os.path.join('', str(user_id) + '.json'), 'r') as file:
